/**
 * Find active patients with payment_method_key = 'quickbooks' and check Healthie recurring payments
 */
...
async function main() {
  ...
  const { rows: patients } = await pool.query<{
    ...
    WHERE status_key = 'active'
      AND payment_method_key = 'quickbooks'
  ...
  console.log(`Found ${patients.length} active QuickBooks patients with Healthie IDs`);
  if (noHealthieId.length > 0) {
    console.log(`Found ${noHealthieId.length} active QuickBooks patients WITHOUT Healthie IDs:`);
  ...
  console.log('\n' + '='.repeat(80));
  console.log('ACTIVE PATIENTS — PAYMENT TYPE: QUICKBOOKS — HEALTHIE RECURRING PAYMENT STATUS');
  console.log('='.repeat(80));
  console.log(`Total QB-payment active patients (with Healthie ID): ${patients.length}`);
  ...
  if (withPackage.length > 0) {
    console.log('\n--- QB PATIENTS WITH HEALTHIE RECURRING PAYMENT ---');
  ...
  if (noPackage.length > 0) {
    console.log('\n--- QB PATIENTS WITHOUT HEALTHIE RECURRING PAYMENT ---');
