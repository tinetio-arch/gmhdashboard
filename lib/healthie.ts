/**
 * Healthie API Client
 * Types imported from lib/healthie/types.ts (single source of truth)
 */
import { healthieRateLimiter } from './healthieRateLimiter';

export type {
  HealthieConfig, HealthieClientData, HealthiePackage, HealthieSubscription,
  CreateClientInput, CreatePackageInput, AssignPackageInput,
  HealthieInvoice, CreateInvoiceInput, HealthiePaymentMethod,
  HealthieMedication, HealthieAllergy, HealthiePrescription, HealthiePharmacy,
  CreateChartNoteInput, HealthieChartNote, HealthieBillingItem,
  HealthieUserRecord, HealthieLocationInput, UpdateClientPayload,
} from './healthie/types';
...
export class HealthieClient {
  ...
  async createClient(input: CreateClientInput): Promise<HealthieClientData> {
    ...
    // Clinic is in Arizona (no DST). Default explicitly to prevent Healthie's
    // practice-level default (Mountain/Denver) from being silently inherited,
    // which causes appointment reminders to be off by 1 hour during DST.
    payload.timezone = input.timezone || 'America/Phoenix';
