...
- ~~Build a multi-form patient experience~~ — ✅ done 2026-05-26. `/ops/intake/[brand]/page.tsx` is a server-rendered hub that platform-routes: iOS/Android → "Open in app / Get the app" hero (deep link `nowoptimal://intake/[brand]`); desktop → multi-form wizard walking through all 7 forms with progress bar, atomic per-form submission to the existing `/api/intake/[brand]/[slug]` endpoint. Single-form URLs still work for direct deep links.
- ~~Add Healthie kill-switch~~ — ✅ done. `INTAKE_PUSH_TO_HEALTHIE=false` makes the patient local-only (skips Healthie create + answer push), status lands as `local_only`. Default is push-enabled; flip per env when moving a brand off Healthie. Verified.
- ~~Wire `Intake Complete` GHL tag on submit~~ — ✅ done 2026-05-26. `lib/intakeGhlTagging.ts` exports `markIntakeCompleteIfDone({brandKey, patientId, applicantEmail, applicantPhone})`. SQL-gated on ALL active brand `form_definitions` being submitted by this patient (statuses `provisioned | healthie_unmapped | local_only`), so we only tag once intake is fully complete — not per-form. `submitIntake()` calls it fire-and-forget on success (both Healthie-on and `local_only` paths). Brand → GHL client mapping in one switch (`abxtac` wired; add others as their GHL locations come online). Verified 5/5: held at 6/7 forms, released at 7/7 forms, attempted GHL lookup (test email correctly absent → returned `ghl_contact_not_found`). Best-effort: any GHL error is logged and swallowed; never blocks intake.
- ~~Build native in-app intake screen~~ — ✅ done 2026-05-26. **Mobile-app repo, branch `claude/claude-task-2254f5ef/abxtac-intake-screen`, commit `405c6c7c`** (in worktree `~/.gemini/antigravity/scratch/mobile-app-intake-wt`). New `src/screens/IntakeWizardScreen.tsx` consumes the same JSON contract as the web hub — identity step → 7 brand forms one at a time, each posted atomically to `/api/intake/[brand]/[slug]` with `source: 'ios'`/`'ipad'`. Reuses the existing `NativeFormRenderer` component (mapper inside the screen translates our `form_fields` shape to its `FormModule` shape). `AppNavigator.tsx` got the React Navigation `linking` config (prefixes `nowoptimal://`, `https://nowoptimal.com`; route `intake/:brand → Intake`) and the Intake screen is registered *outside* the auth gate so a fresh patient with no account can land on it from the email link. Typecheck clean. Universal-link prefix is live in config but only fires once Associated Domains / Android App Links are configured (separate one-time setup); custom scheme works today.
