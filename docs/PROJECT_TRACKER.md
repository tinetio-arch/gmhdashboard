...
- **iPad patient chart audit — Clinical Notes above Appointments + dates everywhere (claude-task-13194086 / row 1_20260517_144413_194086)**: Phil flagged the chart as "very buggy" and asked for three fixes: (1) Clinical Notes must render BEFORE Appointments (was reversed since the 2026-05-20 fix that promoted Appointments to top-of-chart), (2) Appointments should show **last 3 past + all future** with dates (was 15 past), (3) every file/document row should display its upload date. All three handled in `public/ipad/app.js`: `renderChartingTab` now emits Clinical Notes first then Appointments (with header "Appointments (N of M)" when truncated, `pastAppts.slice(0, 3)`); also fixed a latent bug in `showMoreLabDocs` where the lab-queue date used `formatDocDate` — which is scoped inside `renderDocumentsTab` — and would have thrown `ReferenceError` once a patient had >20 lab queue items; both call sites now fall back through `ordered_date → created_at → received_date` and show "No date" when truly empty (top-level `formatDateDisplay` in the showMore path, local `formatDocDate` in the initial render). Forms, pending forms, and documents already rendered dates; verified. Debug 27/27 PASS. Branch `claude/claude-task-13194086/code-task-from-row-1-20260517-144413-194` pushed to origin (PR-ready). **NOT MERGED to local master**: `~/gmhdashboard` has unrelated uncommitted WIP (`app/api/peptides/inventory/`, `public/ipad/app.js` lab-order approval feature, `docs/DEPLOY_CHECK.md`) from a prior session — Phil to merge once that's cleaned up; after merge run `scripts/sync-mobile.sh` to propagate to /mobile bundle, no pm2 restart needed (public/ files served directly).
