...
### Recently shipped
- **2026-05-26 — App-version gate + pre-appointment update nudge (dispatch row 20260526-192910-191f, project `untangling-healthie-communications-from-healthie` Phase 3).** Built the upstream guard that catches patients whose mobile app is missing or too old to receive push — and therefore the sibling Phase-3 "join your video" push (row `20260526-192907-f4ba`) — and nudges them on a non-push channel BEFORE the appointment. Three artifacts: (1) **`lib/app-version-gate.ts`** — pure library: `evaluateAppVersion({appInstalled, versionMax}) → {needsUpdate, status: 'no_app'|'outdated'|'unknown'|'ok', recommendedAction: 'install'|'update'|'none', currentVersion, minSupportedVersion, latestVersion, storeLinks, reason}` plus `compareSemver`, `MIN_SUPPORTED_APP_VERSION` (env-overridable, default `'2.2.0'`, lockstep with the Lambda `APP_VERSION.LATEST`), `LATEST_APP_VERSION`, iOS/Android store URLs. **`unknown` branch never fires** — until the mobile heartbeat that writes `app_version` is wired (separate future task), every active `patient_push_tokens` row has `app_version=NULL` (50/50 on RDS today); treating that as outdated would spam every existing-token patient on day 1. Once mobile starts reporting versions, `unknown` shrinks and `outdated` becomes the live signal — no code change required. (2) **`app/api/cron/app-version-nudge/route.ts`** — new cron (~290 lines): joins `patients` + `v_patient_comms_profile` to find active patients with `healthie_client_id`, `allow_appointments=TRUE`, and at least one non-push channel reachable (`sms_eligible OR email_eligible`); per-patient Healthie GraphQL `appointments(filter:'upcoming')` with the same APPT_QUERY shape used by `appointment-reminders` and `intake-reminders`; filters to telehealth-ish `contact_type` (`telehealth|video|video chat|videochat|video call|phone call`) in a 36–48h pre-appointment window (gives the patient overnight to update before the 24h reminder and the ~1h video-link push); evaluates the gate; if `needsUpdate` builds `CommsEvent { name:'app_version_nudge_pre_appt', category:'appointments', idempotencyKey:'app-version-nudge:<appt_id>', dedupWindowMinutes:0, accountKey, preferredChannel:'sms', templateKey:'app_version_nudge_pre_appt.v1', templateVariables:{first_name, provider_name, appt_type, appt_date, appt_time, contact_type, current_version, min_supported_version, latest_version, recommended_action, gate_status, gate_reason} }` and a `CommsPayload` with distinct SMS + email bodies (SMS: short, signed by Granite Mountain Health, includes iOS store link; email: longer with both iOS + Android links and a versioning line). Calls `notifyPatient()` with `preferredChannel:'sms'` because push is by definition broken for these patients; the gateway falls back to email via normal priority. Per-patient 500ms sleep, 270s deadline. **DRY-RUN BY DEFAULT** per Phil's hard rule "entire comms stack stays gated off real patients until he signs off on testing": `APP_VERSION_NUDGE_DRY_RUN !== '0'` (default `'1'`) → no `notifyPatient()` call, no ledger write, no patient contact; response carries the full `previews: [{patient_id, healthie_client_id, appointment_id, appointment_at, contact_type, account_key, idempotency_key, gate_status, gate_reason, current_version, min_supported_version, recommended_action, title, sms_body, email_subject, email_body, preferred_channel}]` so Phil can `curl … -H 'x-cron-secret: …' | jq` and audit who *would* be nudged. `APP_VERSION_NUDGE_ENABLED=0` kills the cron entirely. Hardcoded telehealth synonym list + same `clientTypeToAccountKey()` mapping the sibling intake-reminders cron uses (audit metadata only). (3) **`app/api/headless/push-tokens/register/route.ts`** — modified to accept an optional `appVersion` field and write it to `patient_push_tokens.app_version` + bump `last_heartbeat_at`; semver-shape regex (`^[0-9]+(\.[0-9]+){0,3}(-[A-Za-z0-9.+-]+)?$`) on the boundary; malformed strings stored as NULL (the gate's `unknown` branch handles those correctly); `ON CONFLICT DO UPDATE` uses `COALESCE(EXCLUDED.app_version, patient_push_tokens.app_version)` so a re-registration without `appVersion` doesn't blank an existing value. **Without this third edit the gate has no signal to fire on** — every active token currently carries `app_version=NULL`. Mobile-side wiring (have `useAppVersionCheck.ts` report current `Constants.expoConfig.version` on register) is a separate mobile-app task, not bundled in this slice. Live verification on RDS: candidate SQL returned **396 eligible patients** (active + reachable on SMS/email + `allow_appointments=TRUE`); **353 with no app**, **43 with app installed but `app_version_max=NULL`** (the safety-valve cohort). Library smoke (`.tmp/gate-smoke.ts`, since deleted): all 7 evaluation cases + 5 `compareSemver` cases PASS. Typecheck: 148 baseline = 148 with these files added (zero new errors). **Sibling-task no-overlap**: claimed files are disjoint from `claude-task-2907f4ba` (video-join push) which owns `lib/video-call-event.ts` + `app/api/video/notify-join/route.ts`. Doc: `docs/APP_VERSION_GATE.md` (new). **NOT deployed and NOT cron-wired yet** — Phil reviews dry-run output, flips `APP_VERSION_NUDGE_DRY_RUN=0`, adds the crontab entry, then later still flips Healthie's native cadence off in the same window per the Phase-4 cutover rule. Branch `claude/claude-task-2910191f/project-launch-from-row-20260526-192910-`.
- **2026-05-26 — iPad Patient-Chart AI Phase 3 UI: slide-over chat (dispatch row 20260527-004147-828c, project `mobile-ipad`).** Wires the staff-facing client to the Phase 2 grounded-Q&A backend (`POST /api/ipad/patient/[id]/ask/`, commit `00e77d9`) and Phase 1 chart assembler (`lib/patientChart.ts`, commit `a4bb196`). Two-file pure-`public/` change — no TS, no rebuild, no PM2 restart needed; goes live on master push + `scripts/sync-mobile.sh` (pre-commit hook auto-runs it). UI additions: (1) **Sparkle "✨ Ask AI" button** in the patient-detail header (`selectPatient` ~line 11765), placed in a flex group next to the existing "Open Chart" button so the action group reads left-to-right Ask-AI → Open-Chart. Pink/violet gradient (`linear-gradient(135deg, #7c3aed → #ec4899)`) to differentiate from the cyan Open-Chart action. (2) **Right-side slide-over panel** (`460px` on iPad, full-width on phones via `@media (max-width: 520px)`) lazy-mounted to `document.body` on first open via `ensurePatientAiPanelMounted()`. Backdrop is a `rgba(0,0,0,0.55)` overlay with `backdrop-filter: blur(2px)`; click-outside closes. Transform-based slide animation (`translateX(100%) → 0`, 220ms cubic-bezier). ESC closes. (3) **Per-patient conversation state** kept in `patientAiConversations[patientId]` (module-scope object) — reopening the panel restores history; clearing via the `↺` icon wipes only that patient's thread. Backend caps to last 6 turns server-side; client sends the full thread and lets the server enforce the cap. The newest user turn goes in `body.question`; only prior turns are passed in `body.conversation` (matching what the Phase 2 route expects). (4) **Empty-state suggestions** — four chip-buttons ("Summarize chart", "Allergy / interaction risks", "What's overdue?", "Active meds") that pre-fill the composer; tapping doesn't auto-send so the clinician can edit first. (5) **Char-by-char reveal** that simulates streaming on top of the non-streaming JSON response (`streamRevealText()` reveals at ~12ms/chunk, chunk-size scales with answer length so a 1500-char answer takes ~2.4s rather than ~5s). True SSE streaming would require modifying the Phase 2 route to use `InvokeModelWithResponseStreamCommand` instead of `InvokeModelCommand` and emit `text/event-stream`; out of scope for this row, noted as a Phase 4 candidate. (6) **Source chips** below each assistant answer (purple-tinted pill per chart section: Demographics, Problems, Regimen, Recent TRT, Recent peptides, Allergies, Labs, Chart notes) so clinicians can audit which chart sections the model grounded on. (7) **Degraded-section warning** — when `meta.degradedSections` is non-empty, an amber inline callout names which sections were unavailable so the answer can be read with appropriate skepticism. (8) **Refusal styling** for `HTTP 403` scope-gate rejections — assistant bubble flips to amber (`rgba(245, 158, 11, 0.08)`) and the refusal text from `data.answer` (already clinician-readable per Phase 2 — "This patient is enrolled in X, not NowMensHealth.Care…") renders as the answer body, so an out-of-scope question looks intentional, not a bug. (9) **Composer UX** — autosizing textarea (max 160px), Enter sends, Shift+Enter newline, send button disabled during in-flight requests, typing-dots animation while waiting. (10) **Lightweight markdown-ish renderer** (`formatPatientAiBody`) — escapes HTML via the existing `sanitize()`, then converts `**bold**` to `<strong>`, runs of `- ` / `* ` / `• ` lines to `<ul>`, and blank-line-separated blocks to `<p>`. Deliberately minimal — Claude tends to answer in short paragraphs and bullets, not full markdown. **iOS WebKit footgun avoided**: POST URL has a trailing slash (`/ops/api/ipad/patient/<id>/ask/`) because `next.config.js` has `trailingSlash:true` and iOS Safari drops the request body on the 308 redirect (the bug fixed for the McKesson supply panel last week, captured in the `iPad Safari 308 body drop` memory pin). Files: `public/ipad/app.js` (+329, sparkle button + state + panel functions + CSS), `public/ipad/style.css` (+326, slide-over styles). Commit `2ad9195` on branch `claude/claude-task-4147828c/project-launch-from-row-20260527-004147-`, merged to master with `--no-ff` as `merge(ipad-ai): … (row 20260527-004147-828c)` after Phil's explicit deploy go-ahead. Pre-deploy ✅ 5/3/0 SAFE TO DEPLOY (warns: pre-existing TS baseline 148/150, branch-not-master pre-merge, `.env.local` symlink for worktree build). `claude-coord debug` ✅ 27/27. JS syntax ✅ via `node --check`. Mobile bundle resync via `scripts/sync-mobile.sh` ran on the master merge commit. Phase 4 (acceptance tests + production-deploy validation including a clinician-driven live test against a real chart) is a separate follow-up task.
- **2026-05-26 — iPad chart appointments parse + filter junk (dispatch row 20260527-041046-6f6f, project `mobile-ipad`).** Phil's complaint: open chart for `philschafer7@gmail.com` (Healthie 12123979) and all 57 appts render `—` for the date and all show up as Past ("No upcoming appointments"). Root cause: `app/api/ipad/patient-chart/route.ts` returned Healthie `appointments` with the raw `date` field — Healthie's `date` is Postgres-timestamp shape (`"2024-04-01 10:30:00 -0700"`) which iOS Safari's `new Date()` can't parse, so `fmtApptDate()` returned `—` and `apptMs()` returned 0 (all sorted as Past). The converter `pgTimestampToUTCISO` from `lib/db.ts` was already used by `app/api/ipad/patient/route.ts` for video-session dates — applied the same converter here. Also filtered out the two flavors of junk Phil sees in the chart: `appointment_type.name === 'Migrated Appointment'` and `pm_status === 'Cancelled'`. Live Healthie verification against 12123979: 57 raw → **5 clean appts** with parseable ISO timestamps (23 Migrated dropped, 33 Cancelled dropped). Backend-only — no `public/ipad/app.js` change, so no `sync-mobile.sh` needed. Files: `app/api/ipad/patient-chart/route.ts` (+8/-2). Branch `claude/claude-task-10466f6f/project-launch-from-row-20260527-041046-`, commit `a7c0149`, merged to master. Pre-deploy ✅ 6/2/0 (warns: pre-existing TS baseline 148/150, unrelated WIP). `claude-coord debug` ✅ 27/27. `pm2 restart gmh-dashboard` clean. Health-check 4/1/3 (same business-KPI baseline — 6 billing holds / 21 zero-stock peptides / 318 cumulative restarts — not regressions). **NOT pushed to origin** — held for Phil per no-master-push rule.
- **2026-05-26 — Video-call-join comms event wired through the gateway (dispatch row `20260526-192907-f4ba`, project `untangling-healthie-communications-from-healthie` Phase 3).** Replaces Healthie's native "join your video call" send with one we control: push-first, SMS/email fallback, secure join URL in every channel that can carry one, every send logged to `patient_communications`. Three artifacts shipped: (1) **`lib/video-call-event.ts`** — `buildVideoCallJoinEvent({appointmentId, joinUrl, patientFirstName?, providerName?, accountKey?, apptStartIso?, preferredChannel?, dedupWindowMinutes?, templateKey?, templateVariables?})` returns `{event, payload}` ready for `notifyPatient()`. `event.name='video_call_join'`, `category='appointments'`, `idempotencyKey='appt:<id>:video_join'` (hard-collapse double-tap of iPad "Start Video" even if `join_url` changes — intentional; outside the 5-min dedup window a follow-up call is treated as fresh), `dedupWindowMinutes=5`, `templateKey='video_call_join.v1'`. Helper doesn't know the join URL — caller supplies it (Healthie `session_id`+`generated_token` URL or Zoom `zoom_join_url`, already fetched by `/api/ipad/patient?action=video_session`). Helper throws on non-http(s) URLs as a defense against `null`/`undefined`/`app://`. Push body is short ("`<provider>` is ready — tap to join.") with the URL in `payload.data.joinUrl` so the mobile app deep-links to its native video screen instead of opening a browser; SMS and email carry the URL inline. Email signed "— Granite Mountain Health". (2) **`app/api/video/notify-join/route.ts`** (POST, basePath `/ops` + trailing-slash required) — `requireApiUser('write')`, validates `patient_id` UUID / `appointment_id` / absolute http(s) `join_url` / optional `account_key ∈ {mensHealth, primaryCare, abxtac}` / optional `preferred_channel ∈ {push, sms, email}`, then composes via the helper and routes through `notifyPatient(..., {source:'api:video.notify-join', actorId: user.user_id})`. **DRY-RUN BY DEFAULT** per Phil's standing rule "entire new comms stack stays gated off real patients until he signs off on testing — NOTHING goes live without thorough testing" (Phase 4 hard gate): `VIDEO_JOIN_NOTIFY_DRY_RUN !== '0'` (default '1') → no `notifyPatient()` call, no ledger write, no patient contact; response carries `dry_run:true` plus `preview:{event, payload}` and `console.log('[VideoJoinNotify] DRY-RUN', {…full preview…})` so Phil can `pm2 logs gmh-dashboard | grep '\[VideoJoinNotify\] DRY-RUN'` to audit composed copy before flipping. `VIDEO_JOIN_NOTIFY_ENABLED=0` returns 503 entirely. (3) **`docs/COMMS_VIDEO_CALL_JOIN.md`** — the per-event standard doc (storage / event spec / curl example / shadow-mode flags / idempotency semantics / verification record / explicit "what this task does NOT do" — Healthie decommission is Phase 4, iPad UI wiring is a separate follow-up, this does NOT build a patient-side video page). **Verification**: helper smoke test (`.tmp/video-call-event-smoke.ts`, 35 assertions) all pass; 7 endpoint HTTP tests against `localhost:3055` cover happy DRY-RUN path + 4 validation 400s + auth 401 + `preferred_channel` pin override; post-test ledger check `SELECT count(*) FROM patient_communications WHERE event_type='video_call_join'` = 0 (DRY-RUN really is dry). Typecheck: zero new errors (worktree 148 = master 148). Sibling-no-overlap note: the COMMS_REMINDER_STANDARD.md "Migration plan" section names this task ID (`20260526-192907-f4ba`) as the appointment-reminder cron migration — that's a separate Phase 3 follow-up, not this task. Per the dispatch inbox brief, this task is exclusively the **video-call-join event**; the 24h-reminder cron migration is its own (still-pending) work item. Pairs with Phase 1 deliverables: `patient_communications` ledger (row `20260526-192859-db1d`) + `notifyPatient()` gateway (row `20260526-192859-78d8`); follows the Phase 2 pattern set by `lib/clinic-reminder-settings.ts` (row `20260526-192906-2008`). Files: `lib/video-call-event.ts` (new, 138 lines), `app/api/video/notify-join/route.ts` (new, 161 lines), `docs/COMMS_VIDEO_CALL_JOIN.md` (new), `docs/PROJECT_TRACKER.md` (this entry), `docs/COMMS_REMINDER_STANDARD.md` (1-line clarification noting the cron-migration row is separate). NOT pushed to origin, NOT deployed (DRY-RUN gated; even if deployed, no real send fires). Branch `claude/claude-task-2907f4ba/project-launch-from-row-20260526-192907-`.
- **2026-05-26 — Jarvis ask-route scope gate (dispatch row 20260527-012352-1888, project `mobile-ipad`).** Phase 2 of the iPad Patient-Chart AI shipped a Q&A route (`app/api/ipad/patient/[id]/ask/route.ts`, commit `00e77d9`) that grounded answers in the open patient's chart but did NOT check whether TRT- or Men's-Health-specific guidance was actually in scope for that patient — a primary-care or longevity patient open in the iPad chart would have gotten TRT supply-status / dispense / regimen guidance just as readily as a NowMensHealth.Care patient. Fix is two-layered: (1) **server-side pre-filter (load-bearing)** — new `detectTRTIntent(question, history)` runs a tight regex (TRT, cypionate/enanthate/propionate, supply status, HCG, gonadorelin, anastrozole/arimidex, enclomiphene/clomiphene, tamoxifen, aromatase-inhibitor, NMH TRT — lab-only terms like "last total T" deliberately excluded) against the inbound question plus the last 6 conversation turns; new `fetchPatientScope(patient_id)` loads `clinic, client_type_key, ghl_tags` from `patients` and computes `isMensHealth` using the same canonical check `app/api/jarvis/peptide-eligibility/route.ts` already uses (clinic substring `'men'`/`'nowmenshealth'` + `client_type_key ∈ {nowmenshealth, qbo_tcmh_180_month, jane_tcmh_180_month, qbo_f_f_fr_veteran_140_month, jane_f_f_fr_veteran_140_month, mens_health_qbo}` + GHL `trt` / `testosterone` / `men's health` tag). Mismatch → HTTP 403 with a clinician-readable refusal naming what the patient IS enrolled in + suggesting enroll-in-MH or route-to-MH-team; rejection written to `agent_action_log` as `status=rejected, summary='Out-of-scope TRT/MH question for non-mens_health patient', details.reason='scope_gate_trt_for_non_mens_health'`. **Fail-closed semantics**: if the scope query throws, the route defaults to `scope unknown` which trips the same gate as a non-MH patient (cannot prove MH → cannot answer MH). (2) **`<patient_scope>` system-prompt block (defense-in-depth)** — Claude now receives `{enrolledPrograms, isMensHealth, isPrimaryCare, isLongevity, isMentalHealth, rawClinic, clientTypeKey}` alongside `<patient_chart>` plus a new SCOPE GATE clause that mirrors the server gate, so any borderline question that slipped past the regex still gets steered toward "out of program scope" instead of confabulated TRT guidance. Lab readings (e.g. "what was the last total T level on this patient") are explicitly NOT gated — primary-care patients legitimately have testosterone labs; the gate is on therapy / regimen / dispense framing only. Files: `app/api/ipad/patient/[id]/ask/route.ts` (+254/-1). Branch `claude/claude-task-23521888/project-launch-from-row-20260527-012352-` ff-merged to master as `merge(ipad-ask): … (row 20260527-012352-1888)`. Pre-deploy ✅ 7/1/0; production build clean (postbuild also ran `sync-mobile.sh`); `pm2 restart gmh-dashboard` clean (pid online, 29s uptime); smoke-test confirmed `POST /ops/api/ipad/patient/<uuid>/ask/` returns `401 Unauthorized` for unauthenticated callers (expected — auth via `requireApiUser`); health-check 4/1/3 (same business-KPI baseline — 6 billing holds / 21 zero-stock peptides / 317 cumulative restarts — not regressions). **iPad UI does NOT yet call this route** (Phase 2 endpoint exists but no client wiring), so the gate is preemptive; when the UI lands it will see refusals as 403 with `data.scopeGate.blocked=true` + `data.answer` already containing the clinician message. **NOT pushed to origin** — left for Phil per no-master-push rule.
- **2026-05-26 — iPad patient-chart audit: Appointments + Clinical Notes (dispatch row 20260527-010538-0157).** Phil: "chart is very buggy — Appointments should show last 3 patient appointments + any future appointments with dates; Clinical Notes should be collapsed by default." Two surgical fixes in `public/ipad/app.js` (mobile bundle resynced to `app.baa4c818.js` by pre-commit hook). (1) Chart-header Appointments strip (`renderChartPanel` ~line 6611) was capped at `upcoming.slice(0, 2)` — so patients with 3+ future visits had appointments silently hidden. Now shows ALL upcoming + last 3 past, both with date/time via `fmtApptDate`. Section labels updated to "Upcoming (N)" / "Last 3" so it's obvious what's rendered. (2) Clinical Notes section in Notes tab (`renderChartingTab` ~line 7804) was a fully-expanded `chart-section` that dominated the panel scroll; flipped to `chart-section collapsed` so it matches the Alerts pattern — header still shows count, click to expand. No backend/API change, no schema change, no pm2 restart needed (public/ static files go live on master push). Verified live: `curl https://nowoptimal.com/ops/ipad/app.js | grep "row 20260527-010538-0157"` → 2 matches; `md5sum` of live /ipad/app.js + /mobile/app.baa4c818.js both equal master HEAD. Pre-deploy ✅ 6/2/0 (warns: pre-existing TS baseline 148/150, unrelated docs/DEPLOY_CHECK.md edit). `claude-coord debug` ✅ 27/27. Commit `21c6bdf` on master, pushed to origin (`ad3bdcf..21c6bdf`); branch `claude/claude-task-05380157/code-task-from-row-20260527-010538-0157-` also pushed. Phil refreshes iPad to see it.
...
- **2026-05-26 — Open all WC peptides to staff on iPad/mobile shop (claude-task-43418e2f, peptide-ops row `20260527-004341-8e2f`).** `app/api/ipad/billing/woo-products/route.ts` previously gated `YPB.250+` SKUs to admin-only (Phil + 2 other admin emails), so regular staff couldn't see or sell newer peptides through the iPad ship/company/in-house flow or the staff `/mobile/` shop (both surfaces consume the same endpoint). Both are auth-gated (`gmh_session_v2` cookie + 'read' role), so per-SKU admin gating on this endpoint added no real security boundary — it just blocked staff from selling. Removed the SKU filter; non-YPB BioBox (B0%) was already unrestricted here. **Patient-facing visibility unchanged** — `/api/headless/products` continues to gate per `app_product_visibility` table (live: `YPB.200-249=all`, `YPB.250+=admin`, `B0%=admin`, `YPB.290=all`), so patients still don't see YPB.250+ on the mobile app. Pricing logic untouched: admin staff still get the `at_cost_price` (wholesale + $10) tier; non-admin staff still default to retail; wholesale-leak debug test (`Non-admin sees 0 wholesale prices`) still passes — that test queries the patient-facing endpoint, which I didn't touch. Diff: 4 ins / 7 del, 1 file. Commit `37ac177` → merged to master `23a79a0` (`--no-ff`). `claude-coord debug` ✅ 27/27. **Deployed** per Phil's approval after closeout — pre-deploy gate ✅ 6/2/0 (first run reported 346 TS errors and BLOCKED; immediate re-run dropped to 148 — under 150 baseline — concurrent state from another session settled), `pm2 restart gmh-dashboard` clean (pid 933371), live `/ops/api/ipad/billing/woo-products/` → 401 (auth-gated as expected), built `route.js` confirms admin filter removed (only remaining `YPB.290` ref is the BIOBOX_SLUG constant import). Health check 4/1/3 same business-KPI baseline (billing holds 6, peptide zero-stock 21, PM2 restart cumulative — all pre-existing, not regressions). Services 16/16 online.
