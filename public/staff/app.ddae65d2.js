...
async function loadSentFaxes(patientId) {
    ...
    try {
        ...
        container.innerHTML = data.faxes.map(function(f) {
            ...
            var sentDate = '';
            try { sentDate = new Date(f.sent_at).toLocaleString('en-US', { timeZone: 'America/Phoenix', month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' }); } catch(e) { sentDate = f.sent_at || ''; }
