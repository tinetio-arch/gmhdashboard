...
// Auto-prints: labels → Dymo, requisition → HP. No extra clicks needed.
function showLabPrintModal(orderId, hasRequisition) {
    // AUTO-PRINT immediately — no button clicks required
    var labelsStatus = '🏷️ Sending 3 specimen labels → Dymo...';
    ...
    (async function() {
        try {
            var result = await apiFetch('/ops/api/labels/print/', {
                method: 'POST',
                body: JSON.stringify({ label_type: 'lab', printer: 'dymo', params: { orderId: orderId } })
            });
            updateModal('✅ 3 specimen labels sent to Dymo (Job #' + result.job_id + ')', document.getElementById('labPrintStatus')?.innerHTML.split('<br>')[1] || '');
        } catch (e) {
            updateModal('❌ Dymo labels failed: ' + (e.message || 'error'), document.getElementById('labPrintStatus')?.innerHTML.split('<br>')[1] || '');
        }
    })();
